...
from app.services.script_quality import PROHIBITED_PHRASE, load_manual_plan, resolve_manual_script_path, validate_and_repair_plan
  ...
  "scenes": [
    {
      ...
      "narration": "¿Qué pasaría si la Luna desapareciera?",
      "subtitle": "distinto"
    }
  ]
}
""",
    ...
    plan = load_manual_plan(manual, JobCreate(topic="¿Qué pasaría si la Luna desapareciera?", duration_seconds=4, scene_duration_seconds=4))

    assert plan.title == "¿Qué pasaría si la Luna desapareciera?"
    assert plan.scenes[0].subtitle == plan.scenes[0].narration


def test_resolve_manual_script_path_falls_back_when_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_job_can_be_created_with_manual_script_path(tmp_path: Path) -> None:
    init_db()
    ...
    with SessionLocal() as db:
        job = worker.create_job(
            ...
            JobCreate(
                ...
                script_path=str(tmp_path / "manual_script.json"),
            ),
        )
        job_id = job.id

...
La fábrica puede enviar `data/jobs/{job_id}/final/final.mp4` por Telegram cuando un job termina correctamente.
...
Si Telegram falla, el job se mantiene `completed`. El error queda en `telegram_error`, `job.json`, `logs/app.log` y `data/jobs/{job_id}/logs/job.log`.
...
El endpoint verifica que el job exista, que `final_video_path` apunte a un archivo real y que Telegram esté configurado.

...
def narration_limit_for_scene(duration_seconds: int) -> int:
    return MANUAL_4S_MAX_CHARS if duration_seconds <= 4 else 180


def narration_min_chars_for_scene(duration_seconds: int) -> int:
    return MANUAL_4S_MIN_CHARS if duration_seconds <= 4 else 1
...
def load_manual_script(script_path: Path) -> ManualScript:
    ...
    errors = _validate_manual_payload(payload)
    if errors:
        raise ScriptQualityError("; ".join(errors))

    scenes_payload = []
    for raw_scene in payload["scenes"]:
        narration = normalize_manual_narration(raw_scene["narration"])
...
def _validate_manual_payload(payload: dict) -> list[str]:
    errors: list[str] = []
    ...
    for index, raw_scene in enumerate(raw_scenes, start=1):
        ...
        cleaned = normalize_manual_narration(narration)

...
class Job(Base):
    ...
    latest_video_path: Mapped[str | None] = mapped_column(Text)
    archive_video_path: Mapped[str | None] = mapped_column(Text)
    script_path: Mapped[str | None] = mapped_column(Text)
    telegram_status: Mapped[str | None] = mapped_column(String(40), default="disabled")
...
class Scene(Base):
    ...
    tts_provider_used: Mapped[str | None] = mapped_column(String(40))
    tts_fallback_used: Mapped[bool | None] = mapped_column(Integer)
    raw_audio_path: Mapped[str | None] = mapped_column(Text)
    normalized_audio_path: Mapped[str | None] = mapped_column(Text)
    raw_audio_duration_seconds: Mapped[float | None] = mapped_column(Float)
    normalized_audio_duration_seconds: Mapped[float | None] = mapped_column(Float)
    error_message: Mapped[str | None] = mapped_column(Text)
